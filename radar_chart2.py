...
class Radar:
    ...
    def _setup_axis(self, facecolor=(1,1,1,0), ax=None):
    ...
    def setup_axis(self, facecolor=(1,1,1,0), figsize=(12, 12), ax=None, **kwargs):
...
max_range=[10, 10, 10])
    ...
    def _draw_radar(self, values, ax=None, **kwargs):
        ...
        unos = vertices[:,0]
        doss = vertices[:,1]
        for an in range(len(dfver)):
            auxtxt = ax.text(unos[an], doss[an], values[an], c='w', fontproperties=prop, zorder=5)
            auxtxt.set_path_effects([path_effects.withStroke(linewidth=1, foreground="#000")])

            
        #print(self.rotation_sin)
        #print(self.rotation_cos)
        #st.write(self.rotation_sin)
